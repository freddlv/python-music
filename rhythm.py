...
class Rhythm:
    ...
    def generate(self, minunit: float, duration: float, multipliers: list, multipliers_probs: list, is_rest_prob: float = 0.5):
        ...
        while currentPos < total_divisions:
            ...
            if choice + currentPos > total_divisions:
                continue
            currentPos += choice
            self.add(Note(isRest, float(choice) * minunit))
